...
import { useRattrapageActions } from "@/hooks/useRattrapages";
...
export default function RattrapageForm() {
  ...
  const onSubmit = (values: FormValues) => {
    ...
    createRattrapage(payload as any, {
      onSuccess: () => {
    ...
    <AppLayout>
      <div className="space-y-8 p-4 md:p-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">Demande de rattrapage</h1>
            <p className="text-muted-foreground">Planifiez une séance de rattrapage pour un cours</p>
          </div>
        </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <CardContent className="space-y-6 pt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField control={form.control} name="type_rattrapage" render={({ field }) => (
                      <FormItem>
                        <FormLabel>Type de rattrapage</FormLabel>
                  ...
                  <FormField control={form.control} name="motif" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input {...field} placeholder="Justification du rattrapage..." />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
              ...
              <div className="flex justify-end gap-4 pt-4">
                <Button type="button" variant="outline" size="lg" onClick={() => navigate('/gestion/rattrapages')}>Annuler</Button>
                <Button type="submit" size="lg" disabled={isCreating}><Save className="mr-2 h-4 w-4" />{isCreating ? "Création..." : "Créer la demande"}</Button>
              </div>
            </form>
          </Form>
        </div>
